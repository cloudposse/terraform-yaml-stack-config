      ...
      components = {
        ...
        terraform = {
          for k, v in stack.components.terraform : k => {
            ...
            workspace = try(v.backend_type, "") == "remote" ? format("%s-%s", format("%s-%s", try(v.vars.environment, module.this.environment), try(v.vars.stage, module.this.stage)), k) : (
              try(v.component, null) != null ? format("%s-%s-%s", try(v.vars.environment, module.this.environment), try(v.vars.stage, module.this.stage), k) : (
                format("%s-%s", try(v.vars.environment, module.this.environment), try(v.vars.stage, module.this.stage))
              )
            )
          }
        }
